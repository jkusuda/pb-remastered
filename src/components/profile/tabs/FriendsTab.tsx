...
export default function FriendsTab({ friends }: { friends: FriendWithUser[] }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col gap-6">
      {accepted.length > 0 && (
        <section>
          <h3 className={`${PIXEL} text-[9px] text-[#3a5a00] mb-3`}>FRIENDS ({accepted.length})</h3>
      ...
      {pending.length > 0 && (
        <section>
          <h3 className={`${PIXEL} text-[9px] text-[#4a6600] mb-3`}>PENDING ({pending.length})</h3>
          <div className="flex flex-col gap-2">
            {pending.map((f) => (
...
function FriendRow({ friend, pending }: { friend: FriendWithUser; pending?: boolean }) {
  return (
    <div className={`flex items-center gap-4 p-3 bg-[#c8e070]/50 rounded-xl border-[2px] ${BORDER}/20`}>
      <img
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-['DM_Sans'] font-semibold text-sm text-[#3a5a00] truncate`}>
          {friend.friend.trainer_name}
        </p>
        <p className={`${PIXEL} text-[7px] text-[#4a6600] mt-0.5`}>
          LVL {friend.friend.level}
        </p>
      </div>
      {pending && (
        <span className={`${PIXEL} text-[7px] text-[#4a6600] bg-[#d4ed7a] px-2 py-1 rounded-lg border border-[#3a5a00]/30`}>
          PENDING
        </span>
      )}
    </div>
