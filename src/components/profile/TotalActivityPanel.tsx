...
// Pokéball icon
const PokeballIcon = () => (
  <div className={`w-6 h-6 rounded-full bg-red-500 border-[3px] ${BORDER} overflow-hidden flex flex-col relative`}>
    <div className="h-1/2 w-full bg-white absolute top-0" />
    <div className="w-full h-[3px] bg-black absolute top-1/2 -mt-[1px]" />
    <div className={`w-2 h-2 rounded-full bg-white border-[2px] ${BORDER} absolute top-1/2 left-1/2 -mt-[4px] -ml-[4px] z-10`} />
  </div>
);
...
export default function TotalActivityPanel({ stats }: Props) {
  ...
  return (
    <div className={`bg-[#9dcd9d] rounded-[8px] border-[4px] ${BORDER} shadow-[4px_4px_0_black] flex flex-col p-3 w-full`}>
      <h2 className="font-bold text-black text-sm mb-3 tracking-wide">TOTAL ACTIVITY</h2>

      <div className="flex gap-3 justify-between">
        {STAT_DEFS.map(({ key, label, Icon }) => (
          <div key={key} className={`flex-1 bg-[#e0f4d9] rounded-[8px] border-[4px] ${BORDER} p-3 flex flex-col items-center shadow-inner justify-between`}>
